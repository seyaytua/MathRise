...
export class Dashboard {
  ...
  renderOverview(stats) {
    ...
    if (elements.completed) elements.completed.textContent = stats.completedLessons;
    if (elements.total) elements.total.textContent = stats.totalLessons;
    if (elements.accuracy) elements.accuracy.textContent = `${stats.accuracy}%`;
    if (elements.attempts) elements.attempts.textContent = stats.totalAttempts;
    if (elements.progress) elements.progress.textContent = `${stats.progressPercentage}%`;
  }
  ...
  renderProgressChart(courseId) {
    ...
    if (lessonStats.length === 0) {
      chartContainer.innerHTML = '<p style="text-align: center; color: var(--text-secondary); padding: 2rem;">まだ学習データがありません</p>';
      return;
    }
  ...
  renderLessonDetails(courseId) {
    ...
    if (!course || Object.keys(course.lessonProgress || {}).length === 0) {
      container.innerHTML = '<p style="text-align: center; color: var(--text-secondary); padding: 2rem;">まだ学習データがありません</p>';
      return;
    }
    ...
    Object.keys(course.lessonProgress).forEach((lessonId, index) => {
      ...
      row.innerHTML = `
        ...
        <td>
          ...
          <span style="font-size: 0.9rem; color: var(--text-secondary);">${accuracy}%</span>
        </td>
        <td>${lastAttempt}</td>
      `;
